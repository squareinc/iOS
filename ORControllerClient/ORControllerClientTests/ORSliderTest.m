...
- (void)testSetValueWithinBounds
{
    ...
    slider.value = 10.0f;
    XCTAssertEqual(slider.value, 10.0f, @"Slider value should be set value");
}
...
- (void)testSetValueOutsideBoundsGetClipped
{
    ORSlider *slider = [[ORSlider alloc] initWithIdentifier:[[ORObjectIdentifier alloc] initWithIntegerId:1] vertical:NO passive:NO thumbImageSrc:nil];
    slider.value = -10.0f;
    XCTAssertEqual(slider.value, 0.0f, @"Slider value should be minimum value when set with a value smaller than minimum");
    
    slider.value = 110.0f;
    XCTAssertEqual(slider.value, 100.0f, @"Slider value should be maximum value when set with a value greater than maximum");
}
...
- (void)testSetValueOnPassiveSliderShouldNotSetValue
{
    ...
    slider.value = 10.0f;
    XCTAssertEqual(slider.value, 0.0f, @"Slider value should not change (stay default) when trying to set value on passive slider");
}
